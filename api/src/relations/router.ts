...
.get("/followers/:user_id", async ({ params, query }) => await relationController.getUserFollowers({
    user_id: params.user_id, page: query.page, limit: query.limit
}), {
    params: t.Pick(relationSchema.params, ["user_id"]),
    query: t.Omit(relationSchema.pagination, ["skip", "followed_user_id"])
})
.get("/followed/:user_id", async ({ params, query }) => await relationController.getFollowedUser({
    user_id: params.user_id, page: query.page, limit: query.limit
}), {
    params: t.Pick(relationSchema.params, ["user_id"]),
    query: t.Omit(relationSchema.pagination, ["skip", "followed_user_id"])
})
.get("/followers/:user_id/total", async ({ params }) => await relationController.getFollowersTotal({
    user_id: params.user_id
}), {
    params: t.Pick(relationSchema.params, ["user_id"])
})
.get("/followed/:user_id/total", async ({ params }) => await relationController.getFollowedUserTotal({
    user_id: params.user_id
}), {
    params: t.Pick(relationSchema.params, ["user_id"])
})
.get("/has-followed/:user_id/:followed_user_id", async ({ params }) => relationController.hasUserFollowed({
