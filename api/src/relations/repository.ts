...
class RelationRepository {
    ...
    async hasUserFollowed(props: Pick<TRelation["add"], "user_id" | "followed_user_id">) {
        const followed = await this.relations.find({ 
            user_id: new ObjectId(props.user_id), followed_user_id: new ObjectId(props.followed_user_id) 
        }).toArray();
    ...
    async startFollowedOneUser(props: Omit<TRelation["add"], "username" | "created_at" | "profile_picture">) {
        const user = await this.users.find(
            { _id: new ObjectId(props.user_id) }, { projection: { email: 0, password: 0 }}
        ).toArray();

        return await this.relations.insertOne({
            created_at: new Date(),
            user_id: user[0]._id,
            followed_user_id: new ObjectId(props.followed_user_id),
